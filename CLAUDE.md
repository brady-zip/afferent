...
```bash
h5i capture commit -m "…" --model claude-opus-4-8 --agent claude-code
```
...
To hold a live back-and-forth with an interactive Codex peer, enter the operator
loop with the `/radio` command, or use the `radio` skill (`$radio`) for a one-off
ask/consult round-trip. **Never** converse by spawning a headless `codex exec` or
`h5i env run … codex` — those are not the live peer.
