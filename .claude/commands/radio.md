---
description: Enter the h5i radio listen loop — stream channel messages via a persistent Monitor and respond autonomously
...
Start **one** Monitor with `persistent: true` and this command. It blocks on the
edge-triggered wake primitive, emits one line per new **inbound** message
(`wait --as <identity>` reads only *your* inbox, so your own sends never wake you),
and re-arms internally. The dedupe guard + `sleep` bound any repeat/spin:
...
- The Monitor runs until the session ends, you call `TaskStop <task-id>`, or a hard
  ...
  between turns.
- If it dies, re-arm it by running this command again (Step 2).
- To stop the radio: `TaskStop <task-id>` (or tell the user it is stopped).
...
- Keep this the only session holding this identity, or you will race another
  session for the same inbox.
